...
public:
    ...
    int solve(int i,vector<int>& prices,int fee,int n,int buy,vector<vector<int>> &dp){
        ...
        int profit = 0;
        if(buy){
            int purchased = -prices[i] + solve(i+1,prices,fee,n,0,dp);
            int notPurchased = solve(i+1,prices,fee,n,1,dp);
            profit = max(purchased,notPurchased);
        }
        else{
            int sell = prices[i] -fee + solve(i+1,prices,fee,n,1,dp);
            int dontSell = solve(i+1,prices,fee,n,0,dp);
            profit = max(sell,dontSell);
        }
        return dp[i][buy]=profit;
    }
    ...
    int maxProfit(vector<int>& prices, int fee) {
        ...
        int buy = 1;
        vector<vector<int>> dp(n,vector<int>(2,-1));
        return solve(0,prices,fee,n,buy,dp);
    }
};
